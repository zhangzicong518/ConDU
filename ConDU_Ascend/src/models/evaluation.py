...
def zeroshot_classifier(classnames, templates, model):
    ...
    for classname in classnames:
        ...
        texts = clip.tokenize(texts).to("npu:0")  # tokenize
        class_embeddings = model.encode_text(texts)  # embed with text encoder
        class_embeddings /= class_embeddings.norm(dim=-1, keepdim=True)
        class_embedding = class_embeddings.mean(dim=0)
        class_embedding /= class_embedding.norm()
        zeroshot_weights.append(class_embedding)
    zeroshot_weights = torch.stack(zeroshot_weights, dim=1).to("npu:0")
...
def zeroshot_eval(model, loader, zeroshot_weights):
    ...
    for i, data in enumerate(tqdm(loader)):
        ...
        images = data["images"].to("npu:0")
        target = data["labels"].to("npu:0")
...
def evaluate(image_classifier, args, val_preprocess):
    ...
    pretrained_model = pretrained_model.to("npu:0")
    if args.session != 0:
        ...
        for file in os.listdir(store_path):
            if file.endswith(".pth") and file != "unified.pth" and file != "prototypes.pth":
                ...
                image_encoder = image_encoder.to("npu:0")
    ...
    for i, dataset_name in enumerate(args.eval_datasets):
        ...
        image_classifier = image_classifier.to("npu:0")
...
def agnostic_evaluate(image_classifier, args, val_preprocess):
    ...
    for i, dataset_name in enumerate(args.eval_datasets):
        ...
        with torch.no_grad():
            all_protos = [torch.stack(proto_set, dim=0).to("npu:0") for proto_set in pretrained_proto_list]
            ...
            for batch in tqdm(test_loader):
                ...
                images = batch_data['images'].to("npu:0")
                labels = batch_data['labels'].to("npu:0")
