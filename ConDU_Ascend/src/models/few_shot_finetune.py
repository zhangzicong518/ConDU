...
def few_shot_finetune(args):
    print("="*100)
    print("Few-shot finetuning")
    print("="*100)
    ...
    total_params_size = sum(p.numel() * p.element_size() for p in params if p.requires_grad)
    print('The number of Total Trainable Parameters:', sum(p.numel() for p in params if p.requires_grad))
    ...
    if hasattr(torch, "npu") and torch.npu.is_available():
        ...
        devices = [0]   # CPU fallback
    print("Using devices", devices)
    model = torch.nn.DataParallel(model, device_ids=devices)

    # text
    texts = [template(x) for x in dataset.classnames]
    texts = clip.tokenize(texts).to(args.device)
    ...
    pretrained_features_sum = torch.zeros(num_classes, feature_dim).to(args.device)
    sample_counts = torch.zeros(num_classes, dtype=torch.int).to(args.device)
